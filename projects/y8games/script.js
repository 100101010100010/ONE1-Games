...
function createTab(makeActive = true) {
    ...
    frame.frame.src = "https://www.y8.com/";
    ...
    frame.frame.addEventListener('load', () => {
        ...
        if (frame.frame.contentWindow.location.href.includes('https://www.y8.com/')) {
            tab.title = "New Tab";
            tab.url = "";
            tab.favicon = null;
        }
...
function updateAddressBar() {
    ...
    const tab = getActiveTab();
    if (bar && tab) {
        bar.value = (tab.url && !tab.url.includes("https://www.y8.com/")) ? tab.url : "";
    }
}
