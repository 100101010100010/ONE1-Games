...
function createTab(makeActive = true) {
    ...
    frame.frame.src = "https://tubitv.com";
    ...
    frame.frame.addEventListener('load', () => {
        ...
        if (frame.frame.contentWindow.location.href.includes('https://tubitv.com')) {
            tab.title = "New Tab";
            tab.url = "";
            tab.favicon = null;
        }
...
function updateAddressBar() {
    ...
    const tab = getActiveTab();
    if (bar && tab) {
        bar.value = (tab.url && !tab.url.includes("https://tubitv.com")) ? tab.url : "";
    }
}
