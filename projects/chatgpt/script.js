...
function createTab(makeActive = true) {
    ...
    frame.frame.src = "https://chatgpt.com";
    ...
    frame.frame.addEventListener('load', () => {
        ...
        if (frame.frame.contentWindow.location.href.includes('https://chatgpt.com')) {
            tab.title = "New Tab";
            tab.url = "";
            tab.favicon = null;
        }
...
function updateAddressBar() {
    ...
    const tab = getActiveTab();
    if (bar && tab) {
        bar.value = (tab.url && !tab.url.includes("https://chatgpt.com")) ? tab.url : "";
    }
}
